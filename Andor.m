...
%     %               ReadOutRate,    Readout rate in MHz   
    ...
    methods
        function Andor = Andor(CCDSettingTemp, AcquisitionMode, ExposureTime, ReadMode, TriggerMode, PreAmpGain, NumberKinetics, NumberAccumulations,CropHeight,CosmicRayFilterOn,ReadOutRate)
            ...
            if nargin < 10
                ReadOutRate = 2;   %1MHz read out rate, 0: 50kHz, 2: 3MHz
            end
            ...
            if ret == 20201
                disp('Can not communicate with Shamrock devices!');
            else
...
%                         CheckWarning(ret);
    ...
    methods
        function saveSIF(Andor,file,comment)
            ...
            movefile([pwd filesep file(1)],file,'f'); %alwasy save the filename as the first letter; need to move it to the distination;
        ...
        function acquireLive(Andor)   %
            ...
            if Andor.ReadMode == 4 %image mode
                ...
                colormap(gray);
                [ret] = SetImage(1, 1, 1, XPixels(1), 1, YPixels(2));
        ...
        function imageData = acquire0(Andor) %acquire in scilence.            
            ...
            if rdmod ==4
                [ret] = SetImage(1, 1, 1, ccdsize(1), 1, ccdsize(2));
            end
